...
3. **Guardrails:** NeMo Guardrails (CPU) on the **default** AI Gateway `/v1/chat/completions` path (intercept plane). Default profile is LLM self-check (`guardrails.nemo.selfCheck.enabled: true`); set `false` to skip those extra Yes/No completions. Direct NeMo API remains available at `nemo.localhost` for debugging.
